...
public class ArticleOutlineAgent {
    ...
    public void generateArticleOutline(ArticleState articleState, Consumer<String> consumer) {
        ...
        //判断用户是否补充描述
        String userDescriptionPrompt = "";
        String userDescription = articleState.getUserDescription();
        if (StrUtil.isNotBlank(userDescription)) {
            userDescriptionPrompt = PromptConstant.ARTICLE_OUTLINE_AGENT_USER_DESCRIPTION_PROMPT.replace("{userDescription}", userDescription);
        }
